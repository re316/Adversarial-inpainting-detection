...
def main(args):
    ...
    saved_inpainted_dir = os.path.join(args.saved_inpainting_dir, args.data_type,args.model_type, 'ckpt_%d'%args.version, '%s_%s_1000000.pth'%(args.data_type, args.model_type))
    ...
    for i in range(args.mask_times):
        ...
        for data_type in ['clean', 'noisy', 'adv']:
            ...
            for j, (image_original, x_composed, y_true) in enumerate(data_loader, 0):
                ...
                mask, image_masked = generate_masked_samples(x_composed, grad_input, mask_ratio=args.mask_ratio, replace_ratio=args.replace_ratio)
                # mask_random, image_masked_random = generate_random_masked_samples(x_composed, mask_ratio=args.mask_ratio)
                mask, image_masked = torch.from_numpy(mask).type(torch.float32).to(device), torch.from_numpy(image_masked).type(torch.float32).to(device)
                with torch.no_grad():
            ...
            loader_dict['%s_%d'%(data_type, i+1)] = DataLoader(TensorDataset(torch.cat(image_ori),torch.cat(composed_images_used)*(1-args.w1)+args.w1*torch.cat(image_ori), torch.tensor(Y_test)),
                                         batch_size=args.batch_size, shuffle=False, num_workers=0)
            del loader_dict['%s_%d'%(data_type, i)], image_ori, composed_images_pre, inpainted_images_used, composed_images_used
        ...
        print('The data length of input, inpainted and composed images',len(All_image_logits), len(All_inpainted_logits), 'and', len(All_composed_logits))
        ...
        l4 = np.sum(np.abs(np.subtract(All_images_pre.numpy(), All_images_in.numpy())), axis=(1,2,3)).reshape((len(All_images_pre), -1))
        l5 = np.sum(np.abs(np.subtract(All_images_pre.numpy(), All_images_co.numpy())), axis=(1,2,3)).reshape((len(All_images_pre), -1))

         # total variation loss
        if mask_time == 0:
            loss_ori = np.mean(np.abs(All_images_pre[:,:,:,1:].numpy()-All_images_pre[:,:,:,:-1].numpy()), axis=(1,2,3)).reshape((-1,1))+np.mean(np.abs(All_images_pre[:,:,1:,:].numpy()-All_images_pre[:,:,:-1,:].numpy()),axis=(1,2,3)).reshape((-1,1))
            loss_2_c = np.mean(np.abs(All_images_co[:,:,:,1:].numpy()-All_images_co[:,:,:,:-1].numpy()), axis=(1,2,3)).reshape((-1,1))+np.mean(np.abs(All_images_co[:,:,1:,:].numpy()-All_images_co[:,:,:-1,:].numpy()),axis=(1,2,3)).reshape((-1,1))
            loss_c = np.concatenate((loss_ori, loss_2_c), axis=1)
        else:
            loss_c = np.mean(np.abs(All_images_co[:,:,:,1:].numpy()-All_images_co[:,:,:,:-1].numpy()), axis=(1,2,3)).reshape((-1,1))+np.mean(np.abs(All_images_co[:,:,1:,:].numpy()-All_images_co[:,:,:-1,:].numpy()),axis=(1,2,3)).reshape((-1,1))
   
        # l: distance between clean and inpainted
        l1_d_all_1 = np.sum(np.abs(np.subtract(All_inpainted_logits, All_image_logits)),axis=1).reshape((len(All_inpainted_logits),-1))
        ...
        l1_d_all_2 = np.sum(np.abs(np.subtract(All_composed_logits, All_image_logits)),axis=1).reshape((len(All_inpainted_logits),-1))
        ...
        for type in values_list:
            ...
            print('the %s  %d mask time, the auc: %.4f, the all auc: %.4f'%(type, mask_time+1, auc_score, auc_all_score))
            with open('./%s/%s_data/%s/message_1.txt'%(args.saved_data_root, args.data_type, args.model_type), 'a') as file:
                file.write('%s features, %s %d mask time, the auc: %.4f, the all auc: %.4f'%(type, args.attack, mask_time+1, auc_score, auc_all_score))
                file.write(' \n')
                file.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack', default='all', type=str, choices=['all', 'FGSM', 'BIM', 'PGD','APGD','CW','Square','AutoAttack', 'DeepFool'])
